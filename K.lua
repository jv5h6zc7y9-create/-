...
local function UpdateViewportCenter()
    ...
    if not menuButtonPosition then
        ToggleButton.Position = UDim2.new(0, ScreenCenter.X, 0, ScreenCenter.Y)
    end
...
local function IsEnemy(targetPlayer)
    if targetPlayer == game.Players.LocalPlayer then return false end
    if targetPlayer.Team and game.Players.LocalPlayer.Team == targetPlayer.Team then return false end
    if targetPlayer.TeamColor == game.Players.LocalPlayer.TeamColor and targetPlayer.TeamColor ~= BrickColor.new("White") then return false end
    if targetPlayer:GetAttribute("Team") and game.Players.LocalPlayer:GetAttribute("Team") == targetPlayer:GetAttribute("Team") then return false end
    return true
end
...
RunService.RenderStepped:Connect(function()
    ...
    if MenuConfig.SilentAimActive then
    ...
    if MenuConfig.AimbotEnabled and closestRoot and closestHead then
        ...
        if isInteracting then
            local targetPart = (aimTarget == "Head") and closestHead or closestRoot
    ...
    elseif MenuConfig.SilentAimActive and closestHead then
        FOVStroke.Color = Color3.fromRGB(30, 255, 30)
        FOVFrame.BackgroundColor3 = Color3.fromRGB(30, 255, 30)
        local targetDir = (closestHead.Position - Camera.CFrame.Position).Unit
        local targetRot = CFrame.lookAt(Camera.CFrame.Position, Camera.CFrame.Position + targetDir)
