--[[
    ================================================================================
    👑 BROSA SYSTEM v6.0 — CYBERPUNK MONOLITHIC HYBRID EXPLOIT HUB (OPTIMIZED PRO)
...
local function IsValidCharacter(char)
    return char and char:FindFirstChild("HumanoidRootPart") and char:FindFirstChildOfClass("Humanoid")
end
...
-- Функция поиска ближайшего противника внутри радиуса FOV или Magnitude (Auto-Grab)
local function GetClosestPlayerToCursor(maxMagDistance)
    local closestPlayer = nil
    local shortestDistance = Hub.Flags.SilentAimFOV
    local shortestMag = maxMagDistance or math.huge
    local useMagnitude = (maxMagDistance ~= nil)

    local myChar = lp.Character
    if useMagnitude and not IsValidCharacter(myChar) then return nil end
    local myRoot = useMagnitude and myChar.HumanoidRootPart

    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= lp and IsValidCharacter(player.Character) then
            local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
            if humanoid.Health > 0 then
                local rootPart = player.Character.HumanoidRootPart
                
                if useMagnitude then
                    local mag = (rootPart.Position - myRoot.Position).Magnitude
...
-- Поиск ближайшего физического незаякоренного предмета (для Auto-Grab)
local function GetClosestUnanchoredProp(maxDistance)
    local myChar = lp.Character
    if not IsValidCharacter(myChar) then return nil end
    ...
    for _, part in ipairs(workspace:GetDescendants()) do
        if part:IsA("BasePart") and not part.Anchored and not part:IsDescendantOf(myChar) and not part:IsDescendantOf(workspace.Terrain) then
            if not part:FindFirstAncestorOfClass("Player") then
...
-- ИДЕАЛЬНЫЙ FLY (ПОЛЕТ): Без диагонального дрифта, мгновенная остановка
SafeConnect(RunService.RenderStepped, function()
    local char = lp.Character
    ...
    if Hub.Flags.Fly and root and hum then
        ...
        if UserInputService:IsKeyDown(Enum.KeyCode.W) then
            flyVel = flyVel + camCFrame.LookVector
            isMoving = true
        end
        ...
        if UserInputService:IsKeyDown(Enum.KeyCode.A) then
            flyVel = flyVel - camCFrame.RightVector
            isMoving = true
        end
        ...
        if isMoving and flyVel.Magnitude > 0 then
            flyVel = flyVel.Unit * Hub.Flags.FlySpeed
        end
        
        if UserInputService:IsKeyDown(Enum.KeyCode.Space) then
            flyVel = Vector3.new(flyVel.X, Hub.Flags.FlySpeed, flyVel.Z)
...
-- ИСПРАВЛЕННЫЙ АВТОЗАХВАТ (AUTO-GRAB) И МОЩНЫЙ ДАЛЕКИЙ БРОСОК (INSANE THROW) через getgc/debug
local GrabRemote = nil
local RemoteLookTimer = 0

local function OptimizeFTAPModules()
    -- Поиск удаленного ивента захвата
    if not GrabRemote then
        GrabRemote = ReplicatedStorage:FindFirstChild("GrabEvent") or ReplicatedStorage:FindFirstChild("Grab") or ReplicatedStorage:FindFirstChild("Throw")
        if not GrabRemote then
            for _, v in ipairs(ReplicatedStorage:GetDescendants()) do
                if v:IsA("RemoteEvent") and (v.Name:lower():find("grab") or v.Name:lower():find("throw")) then
                    GrabRemote = v
                    break
                end
            end
    ...
    -- Подмена максимальной силы броска через сборщик мусора (Insane Throw)
    pcall(function()
        local getgc = getgc or func_getgc
        local debug = debug or {}
        local getconstants = debug.getconstants or getconsts
        local setconstant = debug.setconstant or setconst
        local getupvalues = debug.getupvalues or getupvals
        local setupvalue = debug.setupvalue or setupval

        if getgc then
            for _, v in pairs(getgc(true)) do
                if type(v) == "table" and rawget(v, "MaxThrowForce") or rawget(v, "MaxThrow") then
                    v.MaxThrowForce = 999999
                    if v.ThrowForce then v.ThrowForce = 999999 end
                end
                if type(v) == "function" and not checkcaller() then
                    local scriptName = getfenv(v).script and getfenv(v).script.Name or ""
                    if scriptName:lower():find("throw") or scriptName:lower():find("grab") or scriptName:lower():find("action") then
                        if getconstants then
                            for idx, cnst in ipairs(getconstants(v)) do
                                if type(cnst) == "number" and (cnst == 750 or cnst == 500 or cnst == 1000) then
                                    setconstant(v, idx, 999999)
                                end
                            end
                        end
                        if getupvalues then
                            for idx, upv in pairs(getupvalues(v)) do
                                if type(upv) == "number" and (upv == 750 or upv == 500) then
                                    setupvalue(v, idx, 999999)
                                end
                            end
                        end
                    end
...
-- Хук автоматического вызова симуляции захвата целей (Auto-Grab Logic Execution)
SafeConnect(RunService.Heartbeat, function()
    RemoteLookTimer = RemoteLookTimer + 1
    if RemoteLookTimer % 60 == 0 then
        OptimizeFTAPModules()
    end
    
    -- Реализация Auto-Grab если включен SilentAim/Combat
    if Hub.Flags.SilentAim and GrabRemote then
        local targetPlayer = GetClosestPlayerToCursor(45)
        if targetPlayer and IsValidCharacter(targetPlayer.Character) then
            pcall(function()
                GrabRemote:FireServer(targetPlayer.Character.HumanoidRootPart, targetPlayer.Character.HumanoidRootPart.Position)
            end)
        else
            local targetProp = GetClosestUnanchoredProp(45)
            if targetProp then
                pcall(function()
                    GrabRemote:FireServer(targetProp, targetProp.Position)
                end)
            end
        end
    end
...
SafeConnect(UserInputService.InputBegan, function(input, processed)
    if Hub.Flags.MegaThrow and not processed then
        if input.UserInputType == Enum.UserInputType.MouseButton2 or input.UserInputType == Enum.UserInputType.Touch then
            if not IsValidCharacter(lp.Character) then return end
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= lp and IsValidCharacter(player.Character) then
                    ...
                    if distance < 35 then
                        pcall(function()
                            if GrabRemote then
                                GrabRemote:FireServer(targetRoot, camera.CFrame.LookVector * Hub.Flags.ThrowForce, true)
                            end
...
SafeConnect(RunService.Heartbeat, function()
    local char = lp.Character
    if Hub.Flags.CounterAttack and IsValidCharacter(char) then
                ...
                if dist < 18 then
                    ...
                    elseif Hub.Flags.CounterMode == "В подкарту" then
                        pcall(function()
                            ...
                            task.wait(0.1)
                            if root then root.CFrame = CFrame.new(Hub.Cache.PreviousCoords) end
                        end)
                    end
                end
            end
...
-- ============================================================================
-- [7. СЛЕДУЮЩЕЕ ПОКОЛЕНИЕ UI: NEO-GLOW CYBERPUNK ENGINE (ПОЛНОЕ ИСПОЛНЕНИЕ)]
-- ============================================================================
local CyberUI = {}
...
movementTab:AddToggle({
    ...
    Callback = function(st)
        ...
        if st and lp.Character and lp.Character:FindFirstChildOfClass("Humanoid") then
            pcall(function() lp.Character:FindFirstChildOfClass("Humanoid").WalkSpeed = Hub.Flags.WalkSpeedValue end)
        end
    end
})
...
movementTab:AddSlider({
    ...
    Callback = function(val)
        ...
        if Hub.Flags.WalkSpeedEnabled and lp.Character and lp.Character:FindFirstChildOfClass("Humanoid") then
            pcall(function() lp.Character:FindFirstChildOfClass("Humanoid").WalkSpeed = val end)
        end
    end
})
...
movementTab:AddToggle({
    ...
    Callback = function(st)
        ...
        if st and lp.Character and lp.Character:FindFirstChildOfClass("Humanoid") then
            pcall(function() lp.Character:FindFirstChildOfClass("Humanoid").JumpPower = Hub.Flags.JumpPowerValue end)
        end
    end
})
...
movementTab:AddSlider({
    ...
    Callback = function(val)
        ...
        if Hub.Flags.JumpPowerEnabled and lp.Character and lp.Character:FindFirstChildOfClass("Humanoid") then
            pcall(function() lp.Character:FindFirstChildOfClass("Humanoid").JumpPower = val end)
        end
    end
})
...
-- Перехват __index для стабильного Скрытия Чита (Speed/Jump) от античита Delta
rawMT.__index = newcclosure(function(self, index)
    ...
    if Hub.Flags.SilentAim and self:IsA("Mouse") then
        if index == "Hit" then
            ...
            if targetUnit and IsValidCharacter(targetUnit.Character) then
...
-- Оптимизированное обновление WalkSpeed и JumpPower через RenderStepped во избежание фризов/откатов
SafeConnect(RunService.RenderStepped, function()
    if lp.Character then
        local hum = lp.Character:FindFirstChildOfClass("Humanoid")
        if hum then
            if Hub.Flags.WalkSpeedEnabled and hum.WalkSpeed ~= Hub.Flags.WalkSpeedValue then
                hum.WalkSpeed = Hub.Flags.WalkSpeedValue
            end
            if Hub.Flags.JumpPowerEnabled and hum.JumpPower ~= Hub.Flags.JumpPowerValue then
