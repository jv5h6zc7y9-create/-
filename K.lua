--[[
    TRIPLEWARE MOBILE EDITION - FULL MONOLITHIC SCRIPT
    Все функции сохранены, добавлена поддержка мобильных устройств
    Система сохранения конфигов, открытие/закрытие меню
    Версия: 3.0 Mobile
]]
...
game:GetService("StarterGui"):SetCore("SendNotification", {Title = "Tripleware Mobile", Text = "discord.gg/tripleware", Duration = 10})
...
-- MOBILE DETECTION
-- ============================================
local isMobile = UIS.TouchEnabled or (UIS:GetMouseLocation().X == 0 and UIS:GetMouseLocation().Y == 0)
local isTablet = isMobile and UIS:GetMouseLocation().X > 0
...
local G = {
    ...
    mouse1down = mouse1down or function() end,
    mouse1up = mouse1up or function() end,
...
-- Touch handlers
AC(UIS.TouchBegan:Connect(Safe(function(touch, gameProcessed)
    if gameProcessed then return end
    if UIS:GetFocusedTextBox() then return end
    
    local id = touch.UserInputState
    TouchData.Active = true
    TouchData.StartPos = touch.Position
    TouchData.CurrentPos = touch.Position
    TouchData.StartTime = tick()
    TouchData.Fingers[id] = true
    
    -- 3 finger tap for menu toggle
    local count = 0
    for _ in pairs(TouchData.Fingers) do count = count + 1 end
    if count >= 3 then
        G.MenuVisible = not G.MenuVisible
        if Main then Main.Visible = G.MenuVisible end
        TouchData.Fingers = {}
        return
    end
    
    -- 2 finger tap for aimbot toggle
    if count >= 2 then
    ...
    -- Single finger for aiming
    if count == 1 then
        TouchData.IsAiming = true
        TouchData.AimFingerId = id
    end
end)))

AC(UIS.TouchMoved:Connect(Safe(function(touch, gameProcessed)
    if gameProcessed then return end
    if not TouchData.Active then return end
    TouchData.CurrentPos = touch.Position
    TouchData.Delta = TouchData.CurrentPos - (TouchData.StartPos or TouchData.CurrentPos)
    
    -- Aim assist on touch drag
    if TouchData.IsAiming and Config.Aimbot.Enabled and Config.Aimbot.TouchMode then
        local delta = TouchData.Delta * 0.3
        G.mousemoverel(delta.X, delta.Y)
    end
end)))

AC(UIS.TouchEnded:Connect(Safe(function(touch, gameProcessed)
    if gameProcessed then return end
    TouchData.Fingers[touch.UserInputState] = nil
    if TouchData.AimFingerId == touch.UserInputState then
        TouchData.AimFingerId = nil
    end
    ...
    if count == 0 then
        ...
        TouchData.IsAiming = false
        TouchData.Delta = Vector2.new(0, 0)
    end
end)))
...
local function CreateMobileButtons()
    ...
    MenuBtn.Position = UDim2.new(0.02, 0, 0.02, 0)
    ...
    MenuText.Text = "☰"
    MenuText.Font = Enum.Font.ArialBold
    MenuText.TextSize = 30
    MenuText.TextColor3 = Color3.new(1, 1, 1)
    MenuText.Parent = MenuBtn
    ...
    MobileButtons.MenuBtn = MenuBtn

    -- Aimbot button (🎯)
    local AimBtn = Instance.new("ImageButton")
    AimBtn.Name = "AimBtn"
    AimBtn.Size = UDim2.new(0, 50, 0, 50)
    AimBtn.Position = UDim2.new(0.02, 0, 0.10, 0)
    ...
    AimText.Text = "🎯"
    AimText.Font = Enum.Font.ArialBold
    AimText.TextSize = 26
    AimText.TextColor3 = Color3.new(1, 1, 1)
    AimText.Parent = AimBtn
    
    AimBtn.MouseButton1Down:Connect(function()
        Config.Aimbot.Enabled = not Config.Aimbot.Enabled
        AimBtn.BackgroundColor3 = Config.Aimbot.Enabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
    end)
    
    MobileButtons.AimBtn = AimBtn

    -- Triggerbot button (⚡)
    local TrigBtn = Instance.new("ImageButton")
    TrigBtn.Name = "TrigBtn"
    TrigBtn.Size = UDim2.new(0, 50, 0, 50)
    TrigBtn.Position = UDim2.new(0.02, 0, 0.18, 0)
    TrigBtn.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
    TrigBtn.BackgroundTransparency = 0.15
    TrigBtn.Parent = UI
    Instance.new("UICorner", TrigBtn).CornerRadius = UDim.new(1, 0)
    
    local TrigText = Instance.new("TextLabel")
    TrigText.Size = UDim2.new(1, 0, 1, 0)
    TrigText.BackgroundTransparency = 1
    TrigText.Text = "⚡"
    TrigText.Font = Enum.Font.ArialBold
    TrigText.TextSize = 26
    TrigText.TextColor3 = Color3.new(1, 1, 1)
    TrigText.Parent = TrigBtn
    
    TrigBtn.MouseButton1Down:Connect(function()
        Config.Triggerbot.Enabled = not Config.Triggerbot.Enabled
        TrigBtn.BackgroundColor3 = Config.Triggerbot.Enabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
    end)
    
    MobileButtons.TrigBtn = TrigBtn

    -- ESP button (👁)
    local EspBtn = Instance.new("ImageButton")
    EspBtn.Name = "EspBtn"
    EspBtn.Size = UDim2.new(0, 50, 0, 50)
    EspBtn.Position = UDim2.new(0.02, 0, 0.26, 0)
    ...
    EspText.Text = "👁"
    EspText.Font = Enum.Font.ArialBold
    EspText.TextSize = 26
    ...
    EspBtn.MouseButton1Down:Connect(function()
        ...
        EspBtn.BackgroundColor3 = Config.ESP.Enabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
    end)
    
    MobileButtons.EspBtn = EspBtn

    -- Anti-Aim button (🔄)
    local AABtn = Instance.new("ImageButton")
    AABtn.Name = "AABtn"
    AABtn.Size = UDim2.new(0, 50, 0, 50)
    AABtn.Position = UDim2.new(0.02, 0, 0.34, 0)
    AABtn.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
    AABtn.BackgroundTransparency = 0.15
    AABtn.Parent = UI
    Instance.new("UICorner", AABtn).CornerRadius = UDim.new(1, 0)
    
    local AAText = Instance.new("TextLabel")
    AAText.Size = UDim2.new(1, 0, 1, 0)
    AAText.BackgroundTransparency = 1
    AAText.Text = "🔄"
    AAText.Font = Enum.Font.ArialBold
    AAText.TextSize = 26
    AAText.TextColor3 = Color3.new(1, 1, 1)
    AAText.Parent = AABtn
    
    AABtn.MouseButton1Down:Connect(function()
        Config.AntiAim.Enabled = not Config.AntiAim.Enabled
        AABtn.BackgroundColor3 = Config.AntiAim.Enabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
    end)
    
    MobileButtons.AABtn = AABtn
...
local Config = {
    Aimbot = {Enabled = false, TeamCheck = true, AliveCheck = true, FOV = 50, Smoothness = 4, TargetPart = "Head", WallCheck = true, HoldKey = Enum.KeyCode.LeftAlt, DrawFOV = true, Mode = "Hold", TouchMode = true},
    AntiAim = {Enabled = false, YawOffset = 180, JitterRange = 35},
    Triggerbot = {Enabled = false, HoldKey = Enum.KeyCode.E, Delay = 0.05, TeamCheck = true, Mode = "Hold", TouchMode = true},
    ESP = {
...
-- ============================================
-- UI BUILDING FUNCTIONS (Mobile optimized)
-- ============================================
...
local function CreateMainUI()
    ...
    M.Name = "MainFrame"
    M.Size = UDim2.new(0, math.min(550, UIS:GetMouseLocation().X or 800), 0, math.min(450, UIS:GetMouseLocation().Y or 600))
    M.Position = UDim2.new(0.5, -M.Size.X.Offset/2, 0.5, -M.Size.Y.Offset/2)
    M.BorderSizePixel = 0
    M.Parent = UI
    M.Visible = false
...
-- UI Elements (Mobile optimized with larger touch targets)
local function CreateToggle(p, t, c, k, cb)
    ...
    Bx.Size = UDim2.new(0, 20, 0, 20)
    Bx.Position = UDim2.new(0.9, 0, 0.5, 0)
    ...
    SBtn.MouseButton1Down:Connect(function()
        ...
        mv = UIS.InputChanged:Connect(function(mi) 
            if mi.UserInputType == Enum.UserInputType.MouseMovement or mi.UserInputType == Enum.UserInputType.Touch then
                local pos = mi.Position.X
                local s = math.clamp((pos - SB.AbsolutePosition.X) / SB.AbsoluteSize.X, 0, 1)
                local v = mn + ((mx - mn) * s)
...
local function CreateColorPicker(p, t, c, k, cb)
    ...
    Pv.Size = UDim2.new(0, 24, 0, 20)
    Pv.Position = UDim2.new(0.85, 0, 0.5, 0)
...
local function CreateKeybind(p, t, c, k, cb)
    ...
    BB.Position = UDim2.new(0.65, 0, 0.5, 0)
...
local function CreateSingleDropdown(p, t, opts, c, k, cb)
    ...
    TB.MouseButton1Down:Connect(function()
        ...
        if Lst.Visible and not pop then
            ...
            Lst.CanvasSize = UDim2.new(0, 0, 0, #opts * 20)
        end
        if Lst.Visible then
            local ap = TB.AbsolutePosition
            local mp = Main.AbsolutePosition
            Lst.Position = UDim2.new(0, ap.X - mp.X, 0, (ap.Y - mp.Y) + TB.AbsoluteSize.Y + 2)
            Lst.Size = UDim2.new(0, TB.AbsoluteSize.X, 0, math.min(#opts * 20, 150))
        end
    end)
    return F
end
        ...
        while true do
            if WF and WF.Parent and WF.Visible then
                ...
                if e >= 1 then
                    ...
                    G.lastFPSUpdate = n
                    pcall(function()
                        local device = G.isPhone and "📱" or G.isTablet and "📱" or "💻"
                        WL.Text = string.format("%s tripleware | fps: %d", device, fps)
...
local SkinLeft, SkinRight = CreateSplitTab("Skins")
...
CreateToggle(LegitTab, "Touch Mode", Config.Aimbot, "TouchMode")
...
CreateToggle(LegitTab, "Trigger Touch Mode", Config.Triggerbot, "TouchMode")
...
CreateToggle(MovementTab, "JumpBug", Config.JumpBug, "Enabled")
...
CreateToggle(MovementTab, "EdgeBug", Config.EdgeBug, "Enabled")
...
CreateToggle(MovementTab, "Show Peak", Config.Graph, "PeakEnabled")
...
CreateToggle(VisualsRight, "Skeleton", Config.ESP, "Skeleton")
CreateSlider(VisualsRight, "Skeleton Thickness", 1, 5, Config.ESP, "SkeletonThickness")
CreateColorPicker(VisualsRight, "Skeleton Color", Config.ESP, "SkeletonColor")
local svF = CreateColorPicker(VisualsRight, "Skeleton Visible", Config.ESP, "SkeletonVisibleColor")
visCheckFrames[#visCheckFrames + 1] = svF
svF.Visible = Config.ESP.VisibilityCheck
local shF = CreateColorPicker(VisualsRight, "Skeleton Hidden", Config.ESP, "SkeletonNotVisibleColor")
visCheckFrames[#visCheckFrames + 1] = shF
shF.Visible = Config.ESP.VisibilityCheck
CreateToggle(VisualsRight, "Head Dot", Config.ESP, "HeadDot")
CreateColorPicker(VisualsRight, "Dot Color", Config.ESP, "HeadDotColor")
local dvF = CreateColorPicker(VisualsRight, "Dot Visible", Config.ESP, "HeadDotVisibleColor")
visCheckFrames[#visCheckFrames + 1] = dvF
dvF.Visible = Config.ESP.VisibilityCheck
local dhF = CreateColorPicker(VisualsRight, "Dot Hidden", Config.ESP, "HeadDotNotVisibleColor")
visCheckFrames[#visCheckFrames + 1] = dhF
dhF.Visible = Config.ESP.VisibilityCheck
CreateToggle(VisualsRight, "Highlight", Config.ESP, "Highlight")
CreateColorPicker(VisualsRight, "HL Fill", Config.ESP, "HighlightFill")
CreateColorPicker(VisualsRight, "HL Outline", Config.ESP, "HighlightOutline")
local hvF = CreateColorPicker(VisualsRight, "HL Visible Fill", Config.ESP, "HighlightVisibleFill")
visCheckFrames[#visCheckFrames + 1] = hvF
hvF.Visible = Config.ESP.VisibilityCheck
local hhF = CreateColorPicker(VisualsRight, "HL Hidden Fill", Config.ESP, "HighlightHiddenFill")
visCheckFrames[#visCheckFrames + 1] = hhF
hhF.Visible = Config.ESP.VisibilityCheck
CreateToggle(VisualsRight, "Distance", Config.ESP, "Distance")
CreateColorPicker(VisualsRight, "Distance Color", Config.ESP, "DistanceColor")
CreateToggle(VisualsRight, "Current Weapon", Config.ESP.CurrentWeapon, "Enabled")
CreateColorPicker(VisualsRight, "Weapon Color", Config.ESP.CurrentWeapon, "Color")
CreateButton(VisualsRight, "Reset ESP", function() 
...
CreateToggle(VisualsRight, "Charms", Config.Charms, "Enabled", function(v) 
    if not v then 
...
CreateToggle(VisualsRight, "Charms Team Check", Config.Charms, "TeamCheck", function(v) 
    for plr, parts in pairs(CharmCache) do 
...
CreateColorPicker(VisualsRight, "Charms Visible", Config.Charms, "VisibleColor")
CreateColorPicker(VisualsRight, "Charms Hidden", Config.Charms, "HiddenColor")
CreateSlider(VisualsRight, "Charms Transparency", 0, 1, Config.Charms, "Transparency")
CreateToggle(VisualsRight, "Charms Always On Top", Config.Charms, "AlwaysOnTop")
...
CreateToggle(MiscTab, "Mobile Buttons", Config, "MobileButtons", function(v) 
...
CreateKeybind(MiscTab, "Menu Toggle Key", Config, "MenuToggleKey")
...
-- ============================================
-- CONFIG SYSTEM (File based for mobile)
-- ============================================
CreateLabel(SettingsRight, "-- Config System --", 14, Enum.TextXAlignment.Center)

local CONFIG_DIR = "tripleware_mobile_configs"

local function GetConfigList()
    local list = {}
    pcall(function()
        if listfiles then
            if not isfolder(CONFIG_DIR) and makefolder then makefolder(CONFIG_DIR) end
...
nameInput.Parent = SettingsRight
...
CreateButton(SettingsRight, "Save Config", function()
    ...
    pcall(function()
        if writefile then
            if not isfolder(CONFIG_DIR) and makefolder then makefolder(CONFIG_DIR) end
            local s, d = pcall(DeepCopy, Config)
            if s then 
...
configListFrame.Size = UDim2.new(1, -10, 0, 80)
...
configListFrame.Parent = SettingsRight
...
-- ============================================
-- ESP FUNCTIONS (Preserved from original)
-- ============================================
local ESPFolder
pcall(function()
...
local function ApplySkin()
    ...
    local cur = wm:GetAttribute("TriplewareSkin")
    ...
    wm:SetAttribute("TriplewareSkin", sel)
...
G.EB_StartTime = 0
G.Exploits = {GrenadePrediction = {Enabled = false, LineColor = Color3.new(0, 200, 255), DotColor = Color3.new(255, 0, 0)}}
Config.Exploits = {GrenadePrediction = {Enabled = false, LineColor = Color3.new(0, 200, 255), DotColor = Color3.new(255, 0, 0)}}
...
AC(RS.RenderStepped:Connect(Safe(function()
    ...
    if Config.Aimbot.Mode == "Hold" then
        akd = IsHoldKeyDown(Config.Aimbot.HoldKey) or (Config.Aimbot.TouchMode and TouchData.IsAiming)
    else
        akd = G.AimbotActive
    end
    ...
    if Config.Triggerbot.Mode == "Hold" then
        tkd = IsHoldKeyDown(Config.Triggerbot.HoldKey) or (Config.Triggerbot.TouchMode and TouchData.IsAiming)
    else
...
-- ============================================
-- MOBILE BUTTONS (Create after UI)
-- ============================================
if G.isMobile then
    MobileButtons = CreateMobileButtons()
    if not Config.MobileButtons then
        for _, btn in pairs(MobileButtons) do
...
-- STARTUP
-- ============================================
print("[Tripleware Mobile] Loaded successfully!")
print("[Tripleware Mobile] Tap ☰ button to open menu")
print("[Tripleware Mobile] 3-finger tap to toggle menu")
print("[Tripleware Mobile] 2-finger tap to toggle aimbot")
print("[Tripleware Mobile] Press " .. Config.MenuToggleKey.Name .. " to toggle menu")
